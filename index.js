(() => {
    ...
    Filters.grayscale = function(pixels, effect) {
        ...
        for (let i = 0; i < d.length; i += 4) {
            ...
            let v = (0.3 - (0.3 * effect)) * r + (0.59 - (0.59 * effect)) * g + (0.11 - (0.11 * effect)) * b;
    ...
    //         let b = d[i+2];
    //
    //         let rIntensity = (filter.r * density + 255 * (100 - density)) / 25500;
    //         let gIntensity = (filter.g * density + 255 * (100 - density)) / 25500;
    //         let bIntensity = (filter.b * density + 255 * (100 - density)) / 25500;
    //
    //         let luma = 0.299 * r + 0.587 * g + 0.114 * b;
    //
    //         dst[i] = Math.round(rIntensity * luma);
    //         dst[i+1] = Math.round(gIntensity * luma);
    //         dst[i+2] = Math.round(bIntensity * luma);
    //
    //         dst[i+3] = d[i+3];
    //     }
    //     return output;
    ...
    Filters.separableConvolveFloat32 = function(pixels, horizWeights, vertWeights, opaque) {
        return this.horizontalConvolveFloat32(this.verticalConvolveFloat32(pixels, vertWeights, opaque), horizWeights,
            opaque);
    };
    ...
    Filters.gaussianBlur = function(pixels, diameter) {
        ...
        if (diameter <= 1) {
            return Filters.identity(pixels);
        }
